            ...
            Bcrypt.hash(password, salt, (err, hash) => {
                ...
                user.save((err, user) => {
                    if (!err) {
                        ...
                        try {
                            let templateFile = MailService.getMailTemplate('./lib/user/view/register.mail.ejs');
                            MailService
    ...
    handler: (request, reply) => {
        ...
        User.findOne({email: email}, (err, user) => {
            ...
            if (user && !user.isVerified) {
                ...
                let token = Jwt.sign(tokenData, _privateKey);
                try {
                    let templateFile = MailService.getMailTemplate('./lib/user/view/resend-verification.mail.ejs');
